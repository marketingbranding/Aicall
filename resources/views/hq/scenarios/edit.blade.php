    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4 p-4 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg text-sm">
                Mengedit skenario akan membuat versi baru. Versi sebelumnya tetap tersimpan.
            </div>

            <form method="POST" action="{{ route('hq.scenarios.update', $scenario) }}">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="code" name="code" value="{{ old('code', $scenario->code) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $scenario->name) }}" required
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-sage-500 focus:ring-sage-500 text-sm">
                            <p class="mt-1 text-xs text-gray-500">Nama skenario.</p>
                        </div>
                    </div>
                </div>
