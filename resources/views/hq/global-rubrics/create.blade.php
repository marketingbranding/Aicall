<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('hq.global-rubrics.store') }}" id="rubricForm">
                @csrf
                ...
                const html = `
                    <div class="border rounded-lg p-4 mb-4 bg-gray-50" data-index="${index}">
                        <div class="flex items-center justify-between mb-3">
                            <span class="font-medium text-sm text-gray-700">Item #${index + 1}</span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                            ...
                            <div>
                                ...
                                <input type="number" name="items[${index}][weight]"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-sage-500 focus:ring-sage-500 text-sm"
                                    value="${data.weight || 100}" min="1" max="1000">
                            </div>
                        </div>
                        <div class="mt-3">
                        ...
                        <div class="mt-3">
                            ...
                            <textarea name="items[${index}][evaluation_guidance]"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-sage-500 focus:ring-sage-500 text-sm"
                                rows="2">${data.evaluation_guidance || ''}</textarea>
                        </div>
                        <div class="mt-2">
